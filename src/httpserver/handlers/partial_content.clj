...
(defn get-range [raw-range-header byte-count]
  (let [range-array (get-range-array raw-range-header)]
    (cond
      (= 3 (count range-array)) [(to-long range-array 0) (to-long range-array 2)]
      (= "-" (nth range-array 0)) (range-minus-end (- (to-long range-array 1) 1) byte-count)
...
(defn call [request working-dir]
  {:code 200 :headers {"Content-Type" "text/plain"} :body (get-body request working-dir)})
